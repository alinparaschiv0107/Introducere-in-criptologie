  ...
  for k3 in range(0, 256):
    for k4 in range(0, 256):
      ...
      for ciphertext in ciphertexts:
        xored_ciphertexts.append(hexxor("{:02x}{:02x}".format(k3, k4),ciphertext))
      
      keys = find_keys_partial(xored_ciphertexts[0], plaintexts[0])
      correct_keys = True
      
      for i in range(1, len(plaintexts)):
        key = keys[0] + keys[1] + byte_2_bin(k3) + byte_2_bin(k4)
        y = spn_1r_full_2s(key, str_2_bin(plaintexts[i]))

        if y != hex_2_bin(ciphertexts[i]):
          correct_keys = False
          break

      if correct_keys:
        return (keys[0], keys[1], byte_2_bin(k3), byte_2_bin(k4))
...
def decode_snp_reduced():
	...
	key_str = bytes.fromhex(bin_2_hex(key[0]) + bin_2_hex(key[1])).decode("ascii")
	print("The str key is \"{}\"".format(key_str))

	# Run reduced 2-byte SPN
	xs = str_2_bin(msg)
	ks = str_2_bin(key_str)
	ys = spn_1r_reduced_2s(ks, xs)
	print("Two y halves of reduced SPN: " + ys[0:8] + " (hex: "
		+ bin_2_hex(ys[0:8]) + "), " + ys[8:16] + " (hex: " + bin_2_hex(ys[8:16])
		+ ')')
...
def decode_snp_full():
	...
	print("The key bits: ",key)

	key_str = bytes.fromhex(bin_2_hex("".join(key))).decode("ascii")
	print("The str key is \"{}\"".format(key_str))
...
def main():
  print("--- Decode snp reduced ---")
  decode_snp_reduced()
  print("\n")
  print("--- Decode snp full ---")
  decode_snp_full()
